    ...
    return (
      <div className="h-screen bg-white flex flex-col items-center justify-between py-20 px-10 text-center">
        <div className="flex flex-col items-center">
          <div className="w-24 h-24 bg-emerald-500 rounded-[32px] flex items-center justify-center text-white shadow-2xl shadow-emerald-200 mb-8 animate-in zoom-in duration-700">
        ...
        <button 
          ...
          className="w-full max-w-sm flex items-center justify-center space-x-4 bg-white border border-stone-200 text-stone-700 px-8 py-4 rounded-full font-bold shadow-sm hover:bg-stone-50 active:scale-95 transition-all"
        >
      ...
      <div className="max-w-2xl mx-auto pb-24 space-y-12">
        {view === 'today' && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6">
            ...
            <section onClick={() => setActiveSession(DAILY_MEDITATION)} className="cursor-pointer">
              <div className="aspect-[4/3] bg-emerald-500 rounded-[48px] relative overflow-hidden shadow-2xl group">
        ...
        {view === 'library' && <div className="space-y-10"><h2 className="text-3xl font-black serif">{t.soundscape_mixer}</h2><SoundMixer /></div>}
        {view === 'sleep' && (
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-2 gap-4">
              {MEDITATION_SESSIONS.filter(s => s.category === 'Sleep').map(session => (
                <div key={session.id} onClick={() => setActiveSession(session)} className="aspect-square rounded-[40px] relative overflow-hidden cursor-pointer group">
                  ...
                  <div className="absolute bottom-6 left-6 text-white font-bold">{session.title}</div>
                </div>
              ))}
            </div>
          </div>
        )}
        {view === 'breathing' && <BreathingExercise lang={lang} />}
      </div>
